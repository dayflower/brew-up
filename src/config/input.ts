...
function readInput(getInput: (name: string, options?: { required?: boolean }) => string, name: string, required = false): string {
  return getInput(name, { required }).trim();
}

export function readInputs(getInput: (name: string, options?: { required?: boolean }) => string): RawInputs {
  return {
