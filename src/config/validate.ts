...
const VALID_PUBLISH_MODES = new Set<PublishMode>(['direct', 'pr', 'pr-auto-merge']);
...
function parseReleaseId(releaseId: string): number | undefined {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0) {
    throw new BrewUpError('INVALID_INPUT', 'Input release-id must be a positive integer.');
  }
...
export function validateInputs(raw: RawInputs): ValidatedInputs {
  ...
  if (!TARGET_REPO_PATTERN.test(raw.targetRepo)) {
    throw new BrewUpError('INVALID_INPUT', 'Input target-repo must be in owner/name format.');
  }
