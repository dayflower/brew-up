...
- `publish-title-template` (optional, single-line)
- `publish-body-template` (optional)
- `publish-attribution` (optional: `off` | `commit` | `pr` | `both`, default `both`)

### Behavior

- `publish-title-template` is used for commit subject and pull request title.
- `publish-body-template` is used for commit body and pull request description.
- If omitted, the default title template is `brew-up: update {{output_path}} for {{tag_name}}`.
- If omitted, the default body template is empty.
- Rendering uses Mustache with raw output (no HTML escaping).
- Unknown variables in publish templates are replaced with `UNKNOWN`; rendering does not fail.

### Attribution

When enabled via `publish-attribution`, fixed attribution text is appended:

- commit body: `Generated by brew-up: https://github.com/dayflower/brew-up`
- pull request description: `Generated by [brew-up](https://github.com/dayflower/brew-up)`
...
- `publish-title-template` contains line breaks
- `publish-attribution` has an unsupported value
