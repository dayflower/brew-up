...
describe('parseAssetMap', () => {
  ...
  it('rejects malformed lines', () => {
    expect(() => parseAssetMap('missing_separator')).toThrow(/Invalid asset-map line/);
...
describe('expandAssetPattern', () => {
  it('expands known release variables', () => {
    expect(expandAssetPattern('myapp-{{version}}.zip', variables)).toBe('myapp-1.2.3.zip');
  });

  it('rejects unknown variables', () => {
    expect(() => expandAssetPattern('x-{{unknown}}', variables)).toThrow(/unknown variable/);
  });
});
...
describe('resolveArtifacts', () => {
  ...
  it('fails when no asset matches', () => {
    ...
    expect(() => resolveArtifacts(entries, assets, variables)).toThrow(/matched no release assets/);
  });

  it('fails when multiple assets match', () => {
    const entries = parseAssetMap('default=myapp-{{version}}*.zip');
    expect(() => resolveArtifacts(entries, assets, variables)).toThrow(/matched multiple assets/);
  });
});
