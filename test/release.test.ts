...
import { resolveRelease } from '../src/github/release';
...
describe('resolveRelease', () => {
  it('uses release-id first', async () => {
    ...
    client.rest.repos.getRelease.mockResolvedValue(releaseResponse(1, 'v1.0.0'));
    ...
    expect(client.rest.repos.getRelease).toHaveBeenCalledWith({ owner: 'a', repo: 'b', release_id: 1 });
  ...
  it('uses release-tag when release-id is absent', async () => {
    const client = makeClient();
    client.rest.repos.getReleaseByTag.mockResolvedValue(releaseResponse(2, 'v2.0.0'));
  ...
  it('uses event release id when explicit inputs are absent', async () => {
    const client = makeClient();
    client.rest.repos.getRelease.mockResolvedValue(releaseResponse(10, 'v10.0.0'));
    ...
    expect(client.rest.repos.getRelease).toHaveBeenCalledWith({ owner: 'a', repo: 'b', release_id: 10 });
  });

  it('uses event release tag when event id is absent', async () => {
    const client = makeClient();
    client.rest.repos.getReleaseByTag.mockResolvedValue(releaseResponse(11, 'v11.0.0'));
