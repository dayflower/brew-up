...
| `publish-title-template` | no | Optional Mustache template used for commit subject and PR title (must be single-line) |
| `publish-body-template` | no | Optional Mustache template used for commit body and PR description |
| `publish-attribution` | no | Attribution target: `off`, `commit`, `pr`, or `both` (default: `both`) |
...
Available variables for publish templates:
