...
| `publish-title-template` | no | Optional Mustache template used for commit subject and PR title (must be single-line); see [Title and Body Templates](#title-and-body-templates) |
| `publish-body-template` | no | Optional Mustache template used for commit body and PR description; see [Title and Body Templates](#title-and-body-templates) |
| `publish-attribution` | no | Attribution target: `off`, `commit`, `pr`, or `both` (default: `both`); see [Attribution Modes](#attribution-modes) |
...
## Tap Template Syntax and Variables
...
- For the tap output template (`template-path`), unknown variables cause failure.
- For the tap output template (`template-path`), unresolved placeholders after rendering cause failure.
...
## Publish Templates and Attribution

`publish-title-template`, `publish-body-template`, and `publish-attribution` customize commit and PR text.

### Title and Body Templates

- `publish-title-template` default: `brew-up: update {{output_path}} for {{tag_name}}`
- `publish-body-template` default: empty
- Engine: [Mustache](https://mustache.github.io/) with raw rendering (no HTML escaping)

### Attribution Modes

- `publish-attribution` default: `both`
- `off`: no attribution added
- `commit`: add attribution only to commit body
- `pr`: add attribution only to PR description
- `both`: add attribution to both

Fixed attribution text:

- Commit body: `Generated by brew-up: https://github.com/dayflower/brew-up`
- PR description: `Generated by [brew-up](https://github.com/dayflower/brew-up)`
...
- `release_name`
- `release_url`
- Unknown variables in publish templates are replaced with `UNKNOWN` (publish does not fail)
    ...
    steps:
      ...
      - uses: dayflower/brew-up@v0
        ...
        with:
          ...
          publish-mode: direct
          publish-title-template: "brew-up: update {{output_path}} for {{tag_name}}"
          publish-body-template: "Release {{release_name}} ({{release_url}})"
          publish-attribution: commit
```
    ...
    steps:
      ...
      - uses: dayflower/brew-up@v0
        ...
        with:
          ...
          publish-title-template: "brew-up: update {{output_path}} for {{tag_name}}"
          publish-body-template: "Release URL: {{release_url}}"
          publish-attribution: pr
```
    ...
    steps:
      ...
      - uses: dayflower/brew-up@v0
        ...
        with:
          ...
          publish-title-template: "brew-up: update {{output_path}} for {{tag_name}}"
          publish-body-template: "Resolved release ID: {{release_id}}"
          publish-attribution: both
```
